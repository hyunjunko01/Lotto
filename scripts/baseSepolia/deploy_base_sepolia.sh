...
UPDATE_FRONTEND_ENV="${UPDATE_FRONTEND_ENV_FOR_BASE_SEPOLIA:-false}"
BASE_SEPOLIA_AA_BUNDLER_URL="${BASE_SEPOLIA_BUNDLER_URL:-${BASE_SEPOLIA_RPC_URL:-}}"
...
if [[ "$UPDATE_FRONTEND_ENV" == "true" ]]; then
  ...
  update_env_file "$FRONTEND_ENV_FILE" AA_BUNDLER_URL "$BASE_SEPOLIA_AA_BUNDLER_URL"
...
if [[ "$UPDATE_FRONTEND_ENV" == "true" ]]; then
  ...
  echo "  NEXT_PUBLIC_CHAIN_NAME=Base Sepolia"
  echo "  NEXT_PUBLIC_LOG_LOOKBACK_BLOCKS=10"
  echo "  NEXT_PUBLIC_RPC_URL=$BASE_SEPOLIA_RPC_URL"
  echo "  AA_RPC_URL=$BASE_SEPOLIA_RPC_URL"
  echo "  AA_BUNDLER_URL=$BASE_SEPOLIA_AA_BUNDLER_URL"
else
  echo "Note: frontend/.env.local was not updated. Set UPDATE_FRONTEND_ENV_FOR_BASE_SEPOLIA=true to point the frontend at Base Sepolia."
fi
